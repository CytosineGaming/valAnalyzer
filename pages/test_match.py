...
match_id = "a0645761-ec34-496f-84d0-38609f483558"
selected_round = 1
...
scoreboard_cols = st.columns([0.45,0.3,0.25], gap="medium")
...
with scoreboard_cols[1]:
    ...
    round_event_board = pd.DataFrame(data=format_events(round_events), columns=round_event_headers)
...
for col in range(columns):
    with round_timeline[col]:
        if col == 12 or rounds > 24 and col == 24:
            ...
            st.button(str(round_timeline_data[round_num][0]), on_click=select_round(round_num), use_container_width=True)
